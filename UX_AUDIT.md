...
- [ ] **Render is cropped.** [`Kiosk.tsx:623`](src/components/Kiosk.tsx#L623) draws the
...
- [ ] **The rack vanishes after one try-on.** [`Kiosk.tsx:820`](src/components/Kiosk.tsx#L820)
  ...
  text link.
- [ ] **The progress bar is invisible.** [`Kiosk.tsx:469`](src/components/Kiosk.tsx#L469)
...
- [ ] **The bar lies about finishing.** [`Kiosk.tsx:412`](src/components/Kiosk.tsx#L412) has
...
- [ ] **Seven competing controls in the result bar.**
...
- [ ] Rewrite the "What we keep, and where" bullet to describe actual behaviour.
- [ ] Rewrite "Your choices" ([`privacy/page.tsx:44`](src/app/privacy/page.tsx#L44)) —
  it describes device-only deletion while `clearAllLooks` deletes from cloud storage.
- [ ] Do the same in the Nepali copy ([`privacy/page.tsx:64`](src/app/privacy/page.tsx#L64),
  [`:75`](src/app/privacy/page.tsx#L75)).
- [ ] Verify the "expires on its own after 7 days" claim holds for the cloud copy.
...
- [ ] Add a reset flow. Anyone who forgets is permanently locked out of their looks,
...
- [ ] Accept `?mode=signup`, or default `/login` to signup. A new vendor currently
...
- [ ] [`PendingReview.tsx:561`](src/components/PendingReview.tsx#L561) says *"we can't
...
- [ ] The sign-out button is bare `ph-btn`
...
- [ ] The comment two lines above the light value describes "a near-black green".
- [ ] The dark block's comment requires it be *"cool enough to stay the one thing in a
  brown room that isn't brown"* — `#bbb288` is brown-family khaki.
- [ ] The alpha channel means every `--violet` fill composites with whatever is behind
...
- [ ] Nothing traps focus, moves focus in, restores focus on close, or locks body scroll.
- [ ] Backdrop-click-to-close is universal — including
...
- [ ] `LookViewer` closes when you tap the photo itself — users tap photos to zoom.
- [ ] Build one `<Dialog>` with dialog semantics, focus trap, Escape, scroll lock and a
...
- [ ] Plan limits and approval status —
  [`dashboard/page.tsx:83-91`](src/app/dashboard/page.tsx#L83-L91),
  [`:117-125`](src/app/dashboard/page.tsx#L117-L125)
- [ ] Shop-settings save failure — [`dashboard/page.tsx:286`](src/app/dashboard/page.tsx#L286)
- [ ] Kiosk photo-read failure — [`Kiosk.tsx:306`](src/components/Kiosk.tsx#L306),
  [`KioskV2.tsx:332`](src/components/KioskV2.tsx#L332)
- [ ] Image-read failure in both dashboard modals —
  [`Dashboard.tsx:839`](src/components/Dashboard.tsx#L839),
  [`:945`](src/components/Dashboard.tsx#L945)
- [ ] Kiosk-launch guard — [`Dashboard.tsx:165`](src/components/Dashboard.tsx#L165)
- [ ] Every destructive `confirm()` across account, LooksGallery, LookViewer,
  Dashboard, FabricStudio.

## T1.9 — Failures are silently swallowed

- [ ] **8 × `catch {}`** on optimistic writes in
...
- [ ] **No `catch` at all** in `requestPlan` / `requestCredits`
...
- [ ] Order-fetch errors become "No orders yet"
...
- [ ] `aria-label="try fit 1"` vs visible "fit 01"; when active the visible label
  becomes "on you ✓" while the aria-label stays "try fit 1"
  ([`:71-73`](src/components/HeroTryOn.tsx#L71-L73)).
- [ ] Feed cards use raw `<img>` ([`:240`](src/app/page.tsx#L240)) while the rest of the
  app routes through `GarmentImage`.
- [ ] The "peeq it" pill only appears on `:hover`
...
- [ ] Nav layout jumps: `{shops.length > 0 && <a href="#shops">}` conditionally adds a link.
- [ ] Footer has one link (privacy). No terms, contact, or about.

## `/owner` — [`owner/page.tsx`](src/app/owner/page.tsx)

- [ ] "book a free demo call" goes to `cal.com/contractorops/15min`
  ([`:130`](src/app/owner/page.tsx#L130)) — a vendor clicking the primary trust CTA
  lands on a page branded "contractorops".
- [ ] Six full-bleed screenshots, all raw `<img>`, none lazy-loaded, none with
  ...
  on load.
- [ ] Nav shows "vendor log in" *and* the AccountMenu avatar
  ([`:29-30`](src/app/owner/page.tsx#L29-L30)).
- [ ] Footer drops the privacy link the landing footer has.
- [ ] Step circles are 38px/17px on landing, 36px/16px here. Body copy runs
  14 / 14.5 / 15 / 15.5px across four sections.

## `/signin` + `/login` — [`AuthPage.tsx`](src/components/AuthPage.tsx)

- [ ] Placeholder-as-label on both fields ([`:176`](src/components/AuthPage.tsx#L176),
  [`:180`](src/components/AuthPage.tsx#L180)) — once you type, the field is unlabelled.
- [ ] `aria-invalid` set but no `aria-describedby` tying error text to the input, and
  field errors have no `role` ([`:179`](src/components/AuthPage.tsx#L179),
  [`:184`](src/components/AuthPage.tsx#L184)) — never announced.
- [ ] No password-reveal toggle. No autofocus.
- [ ] "An account with this email already exists — sign in below" renders in
  `role="alert"` + `var(--danger)` ([`:193`](src/components/AuthPage.tsx#L193)) —
  informational message styled as failure.
- [ ] Raw Supabase `error.message` surfaced to users
  ([`:150`](src/components/AuthPage.tsx#L150)).
- [ ] Nothing links to `/privacy` from either auth surface; no terms acceptance on signup.
- [ ] `var(--sage)` / `var(--cream)` / `var(--forest-deep)` here vs
  `--paper` / `--card` / `--ink` on shopper pages.

## `/account` — [`account/page.tsx`](src/app/account/page.tsx)

- [ ] Bespoke header ([`:117-127`](src/app/account/page.tsx#L117-L127)) instead of
  `.efc-nav`. No AccountMenu, and the wordmark isn't a link home — it is everywhere else.
- [ ] Heart pill is `rgba(255,255,255,.9)` with `color: var(--stone)`
...
- [ ] "delete everything" is a 12.5px grey underlined link in `--stone`
  ([`:182`](src/app/account/page.tsx#L182)) — the most destructive action on the page is
  quieter than "share". `--danger` exists.
- [ ] "not now" on the merge banner only does `setPendingLooks(0)`
  ([`:145`](src/app/account/page.tsx#L145)) — not persisted, so it nags next visit.
- [ ] "saved ✓" swaps the button label for 1600ms with no `aria-live`
  ([`:170`](src/app/account/page.tsx#L170)); `saveContact` failure is unhandled.
- [ ] Order status "confirmed by shop" uses `var(--violet)`
  ([`:309`](src/app/account/page.tsx#L309)) — `--ok` is the token for this.
- [ ] Dates hardcoded `"en-GB"` ([`:303`](src/app/account/page.tsx#L303)) in a
  bilingual EN/NE product.
- [ ] Placeholder-as-label on name and phone ([`:159`](src/app/account/page.tsx#L159),
  [`:164`](src/app/account/page.tsx#L164)).
- [ ] Look cards use `borderRadius: 16` while sibling sections use `--radius-card` (20).

## `/s/[slug]` storefront — [`StorefrontClient.tsx`](src/app/s/[slug]/StorefrontClient.tsx)

- [ ] **The same 4 photos appear three times on one page.**
  ...
  it has 2.
- [ ] **Two sort options do nothing different**: "Sort: Featured" and "Newest first"
  both fall through to `() => 0`
  ([`:105-108`](src/app/s/[slug]/StorefrontClient.tsx#L105-L108)). Only one option is
  prefixed "Sort:".
- [ ] **Dark-mode invisible text**: the footer "ask" input sets
...
- [ ] **404 is a dead end** ([`:73-79`](src/app/s/[slug]/StorefrontClient.tsx#L73-L79))
...
- [ ] Nav category links are `<a href="#collection">` that also set filter state
  ([`:139`](src/app/s/[slug]/StorefrontClient.tsx#L139)) — no active state, unlike the
  chips below doing the same job.
- [ ] Nav jumps: the "saved (n)" button appears the instant you heart something
  ([`:151`](src/app/s/[slug]/StorefrontClient.tsx#L151)).
- [ ] Search has no label, no clear button, no `aria-live` result count.
- [ ] Out-of-stock cards dim the whole card to `opacity: 0.6`
  ([`storefront.tsx:55`](src/components/storefront.tsx#L55)) — name and price fail
  contrast — plus a "sold out" button in `--line` on `--stone`.
- [ ] Every card carries both "see it on you →" and a redundant "details" link
  duplicating the card and title links
  ([`storefront.tsx:88-97`](src/components/storefront.tsx#L88-L97)).
- [ ] Announce bar claims "no account needed" above a checkout that pitches signup.

## Cart / checkout — [`storefront.tsx`](src/components/storefront.tsx)

- [ ] Backdrop-click closes the drawer mid-checkout and destroys typed details (T1.7).
- [ ] **Enter in the phone field places the order**
  ([`:388`](src/components/storefront.tsx#L388)) — no confirm; Enter in the name field
  does nothing; it isn't a `<form>` at all.
- [ ] `CheckoutSignIn` is a full email+password signup in a drawer with no privacy
  link, no reset, no reveal.
- [ ] Its messages use `fieldErrorStyle` overridden to `color: var(--stone)`
  ([`:510`](src/components/storefront.tsx#L510)) — errors in grey, everywhere else in
  `--danger`.
- [ ] Its toggle reads "sign in", then becomes **"not now"** when expanded
  ([`:499`](src/components/storefront.tsx#L499)) — implies dismissal, actually collapses.
- [ ] The done screen header says "order sent"
...
- [ ] **Nav parity broken with the storefront**: plain `account`/`sign in` link instead
...
- [ ] Breadcrumb's category segment isn't a link
  ([`:121`](src/app/s/[slug]/[garment]/ProductClient.tsx#L121)).
- [ ] Out of stock hides sizes, qty, add-to-bag *and* try-on, leaving a near-empty page
...
- [ ] Size buttons aren't a radiogroup, have no `aria-pressed`, and the "please pick
...
- [ ] One photo per product, no zoom, no gallery.

## Kiosk (both versions)

- [ ] **The upload dropzone is a `<div onClick>`**
...
- [ ] That box has a 2px dashed border and **no `onDrop` handler**. Dashed border is
...
- [ ] **No way to cancel a generation.** A mis-tap costs the shopper 1–2 minutes and
  the vendor a credit.
- [ ] The generating overlay has **no `aria-live`** — total silence for the whole wait.
- [ ] **Hold-to-compare is pointer-only** ([`Kiosk.tsx:628`](src/components/Kiosk.tsx#L628),
  [`KioskV2.tsx:870`](src/components/KioskV2.tsx#L870)) — no keyboard equivalent.
- [ ] **No browser-history integration.** Both kiosks are `position: fixed` shells with
...
- [ ] `?shared=1` is the only way to put a shop tablet into shared mode
...
- [ ] v2 mixes two accents in one room: attract kicker, `/account` link, dashed upload
...
- [ ] v2's `WantThisSheet` "added ✓" flips back to "add to bag" after 2200ms while the
  sheet is open ([`KioskV2.tsx:800`](src/components/KioskV2.tsx#L800)) — the button
  jitters under the thumb.
- [ ] `FindMySizeSheet` disables save at `opacity: .6` with no message explaining why
  ([`parts.tsx:391`](src/components/kiosk/parts.tsx#L391)). Type 300cm and nothing tells
  you anything.
- [ ] The AI-result disclaimer is 11px ([`Kiosk.tsx:762`](src/components/Kiosk.tsx#L762)).
- [ ] `/kiosk` "nothing listed yet" and `/k/[slug]` "shop not found" offer no
  navigation out.

## `/dashboard` — [`Dashboard.tsx`](src/components/Dashboard.tsx)

- [ ] **Apparel vendors have no link to their own storefront.** "view storefront" only
  renders in the non-apparel branch ([`:184`](src/components/Dashboard.tsx#L184));
  apparel shops must dig the URL out of Settings.
- [ ] **Clicking a garment's photo opens its QR modal**
...
- [ ] Everything on that card is under 12px: code 10.5, name 11.5 with `.12em`
  tracking, size chips 10, actions 11.
- [ ] **The "orders to call back" alert is invisible as an alert**
  ...
  `--mut`. `--warn` exists.
- [ ] The two kiosk buttons diverge on the same guard: "launch kiosk" alerts then
  switches tab, "try v2" silently switches tab
  ([`:163-177`](src/components/Dashboard.tsx#L163-L177)).
- [ ] `sign out` sits immediately left of the primary CTA as 12px grey text
  ([`:154`](src/components/Dashboard.tsx#L154)).
- [ ] `GarmentModal` / `FabricModal`: placeholder-only labels, `canSave` needs only
...
- [ ] **[`LocationPicker.tsx:38`](src/components/LocationPicker.tsx#L38) creates its map
...
- [ ] The map is "Optional" here and unlabelled in Settings
...
- [ ] The nav re-implements `.tabs` from scratch and uses a `--butter-deep` underline
...
- [ ] Urgency has three vocabularies: queues use `--butter-deep`
  ([`admin/page.tsx:318`](src/app/admin/page.tsx#L318)), errors use `--danger`, meters
  use `--warn`.
- [ ] `Queue` cards are `<Link>`-wrapped divs with no hover or focus styling
  ([`admin/page.tsx:313`](src/app/admin/page.tsx#L313)).
- [ ] `forbidden` / `unauth` render in `--mut` grey via `Note`
  ([`:107-118`](src/app/admin/AdminShell.tsx#L107-L118)) — an access denial styled as a hint.
- [ ] "← dashboard" sends an admin to `/dashboard`, which for an admin with no shop
  triggers vendor onboarding.

## `/compare` — [`CompareClient.tsx`](src/app/compare/CompareClient.tsx)

- [ ] **Public route, no auth**, and `/api/compare` spends real OpenAI + fal credits.
  "dev tool · not linked anywhere" is not access control.
- [ ] The upload control is a `<label>` wrapping a hidden input
...
- [ ] `<h1>` → `<h3>`, skipping h2 ([`:124`](src/app/privacy/page.tsx#L124)).
- [ ] Nepali users get a flash of English before `localStorage` is read on mount
  ([`:88-94`](src/app/privacy/page.tsx#L88-L94)).
- [ ] Footer has no links at all — not even home.

---

# Tier 3 — systemic

- [ ] **Pinch-zoom disabled sitewide.** `maximumScale: 1, userScalable: false`
  ([`layout.tsx:21-22`](src/app/layout.tsx#L21-L22)) — WCAG 1.4.4, on a shopping site
  where people zoom into fabric. Delete both lines.
- [ ] **13 hardcoded radius values** despite 3 radius tokens: `999`×72, `2`×17, `6`×15,
  `14`×11, `12`×11, `5`, `20`, `18`, `3`, `8`, `16`, `4`, `11`. Inputs alone are 12px,
  14px and `--radius-btn` (20px) on three different pages.
- [ ] **Two token vocabularies.** Vendor/admin surfaces run on
...
- [ ] **66 raw `<img>`, 1 file importing next/image** — including the product hero and
  all six `/owner` screenshots.
- [ ] **16 × `100vh`**, not `dvh`/`svh`. `.k-stage` uses `52vh` at desktop and `56svh`
  under 640px — inconsistent within one rule.
- [ ] **8px carousel dots** ([`globals.css:492`](src/app/globals.css#L492)) — WCAG 2.5.8
  wants 24px.
- [ ] **Hover-only affordances.** Carousel arrows are `opacity: 0` until hover *and*
  ...
  tooltips.
- [ ] **`'Baloo 2', cursive`** in 11 rules. If Baloo fails to load, every heading and
  button falls back to Comic Sans / Apple Chancery. Use `sans-serif`.
- [ ] **Fonts via CSS `@import`** ([`globals.css:1`](src/app/globals.css#L1)) — blocking
...
- [ ] **Z-index has no scale**: 40 kiosk, 50 garment/fabric/QR modals, 55 studio +
